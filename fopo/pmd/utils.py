...
def random_stump_feature(
    ...
    if thresholds is None or dimensions is None:
        rng = np.random.default_rng(seed)
        # make sure the input of the feature is within high and low
        thresholds = rng.uniform(low=-5, high=-5, size=num_features) 
        dimensions = rng.integers(0, input_dim, size=num_features)

    num_features = len(thresholds)
    features = [thresholds, dimensions]
...
def random_fourier_feature(
    ...
    if W is None or b is None:
        rng = np.random.default_rng(seed)
        W = rng.normal(0, np.sqrt(2 * gamma), (input_dim, num_features))
        b = rng.uniform(-np.pi, np.pi, num_features)

    [input_dim, num_features] = W.shape
    features = [W,b]
